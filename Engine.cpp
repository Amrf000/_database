...
*/
struct Comparator {
	bool operator() (const std::vector<char>& lhs, const std::vector<char>& rhs) const
	{
...
//int inde = 0;
BOOL sortViewTextByWords(HWND view)
{
	...
	std::map<std::vector<char>, std::map<std::vector<char>,char,Comparator>,Comparator> lineCache;
	std::vector<char> nodeName;
	std::vector<char> preline;
	...
	for (int lineNum = 0; lineNum < docLines; ++lineNum)
	{
		...
		if (lineEnd - lineStart)
		{
			...
			if(lineLen>0){
				if (line[0]=='{') {
					nodeName = preline;//上一行字段为节点名
				}
				else if (line[0] == '}') {//下一行是节点
					preline.clear();
					continue;
				}
				else {
					if (preline.size()) {
						lineCache[nodeName][line] = '1';
					}
					else {
						preline.assign(line.begin(), line.end());
					}	
				}
			}
		}
	...
	for (std::map<std::vector<char>, std::map<std::vector<char>, char, Comparator>, Comparator>::iterator it=lineCache.begin();
		 it!=lineCache.end();++it) 
	{
		 const int lineLen0 = it->first.size() - 1;
		 ofs.write(&it->first[0],lineLen0);
		 ofs << std::endl << '{' << std::endl;
		 for (std::map<std::vector<char>, char, Comparator>::iterator ir=(it->second).begin();
			 ir!=it->second.end();++ir) 
		 {
			 const int lineLen1 = ir->first.size() - 1;
			 ofs.write(&ir->first[0], lineLen1);
			 ofs << std::endl;
		 }
		 ofs << "}" << std::endl;
	}
	...
	::SendMessage(view, SCI_SETTEXT, 0, (LPARAM)ofs.str().c_str());
